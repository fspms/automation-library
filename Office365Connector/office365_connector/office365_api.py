...
class Office365API:
    ...
    def activate_subscription(self, content_type: str) -> bool:
        """
        Activate a subscription for a tenant

        :param str content_type: The content type of the subscription
        :return: True if the subscription was activated
        :rtype: bool
        """
        ...
        with self._fresh_session() as session:
            ...
            response = session.post(f"{base_url}/subscriptions/start", params=params)
            ...
            if "error" in subscription:
                raise FailedToActivateO365Subscription(
                    error_code=subscription["error"].get("code"), error_message=subscription["error"].get("message")
                )

            return subscription["status"] == OFFICE365_ACTIVE_SUBSCRIPTION_STATUS
